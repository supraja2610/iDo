...
const string CommandProcessor::FILENAME_VIEW_ALTERNATES = "viewlist.txt" ;
...
void CommandProcessor::initViewList() {
	int viewIndex = 0 ;
	int i ;
	fileObject.setFileName(FILENAME_VIEW_ALTERNATES) ;
	strcpy(viewList[viewIndex],"view") ;
	viewIndex++ ;
	vector<string> storedviewList = fileObject.readFromFile() ;
	vector<string>::iterator it ;
	it = storedviewList.begin() ;
	
	for(; it != storedviewList.end() ; it++) {
		strcpy(viewList[viewIndex],(*(it)).c_str()) ;
		viewIndex++ ;
	}

	strcpy(viewList[viewIndex],"-1") ;
}
...
bool CommandProcessor::actualKeyWord(char userCmd[MAX_COMMAND_SIZE]) {
	...
	bool isAdd, isRemove, isEdit, isSearch, isExit, isUndo,isRedo, isAlternate, isView ;
	...
	isView =  CommandProcessor::isFound(userCmd, "view") ;
	if(isAdd) {
		strcpy(userCmd, "add");
	}
	...
	if(isAdd == false && isRemove == false && isEdit == false && isSearch == false && isExit == false && isUndo == false && isRedo==false && isAlternate == false && isView == false){
		return false;
	}
